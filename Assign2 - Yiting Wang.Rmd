...
expected_p == p
```
Imagine a city that has three blocks with parking meters, two of which will be sampled at random. The true $p$ can be computed using all data. The $E[\widetilde p]$ can be computed using the average of three $\widetilde p$ from three different samples(block 1&2, block 1&3, block2&3). Although the two numbers are very close, they are not the same. So $\widetilde p$ is not an unbiased estimator for p.
...
If we use bootstrapping, we can easily calculate the level of sampling error (margin of error???)
